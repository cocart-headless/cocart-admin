...
class Notices {
	...
	public function hide_notices() {
		if ( isset( $_GET['cocart-hide-notice'] ) && isset( $_GET['_cocart_notice_nonce'] ) ) {
			...
			wp_safe_redirect( remove_query_arg( array( 'cocart-hide-notice', '_cocart_notice_nonce' ), Help::cocart_get_current_admin_url() ) );
			exit;
		}
	} // END hide_notices()
	...
	public function base_tables_missing_notice() {
		...
		include_once dirname( __FILE__ ) . '/views/html-notice-base-table-missing.php';
	...
	public function check_woocommerce_notice() {
		if ( ! defined( 'WC_VERSION' ) ) {
		...
		} elseif ( version_compare( WC_VERSION, Core::$required_woo, '<' ) ) {
			/**
			 ...
			 */
			include_once dirname( __FILE__ ) . '/views/html-notice-required-wc.php';
		}
	...
	public function plugin_review_notice() {
		...
		if ( ( intval( time() - self::$install_date ) ) > WEEK_IN_SECONDS * 2 ) {
			include_once dirname( __FILE__ ) . '/views/html-notice-please-review.php';
		}
