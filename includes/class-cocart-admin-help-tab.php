		...
		if ( isset( $_GET['page'] ) && strpos( trim( sanitize_key( wp_unslash( $_GET['page'] ) ) ), 'cocart' ) === 0 ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			$screen->add_help_tab( array(
				...
				'content' =>
					...
					'<p>' . sprintf( __( 'We are fanatical about support, and want you to get the best out of our REST API with %s. If you run into any difficulties, there are several places you can find help:', 'cart-rest-api-for-woocommerce' ), 'CoCart' ) . '</p>' .

					'<ul>' .
					'<li>' . sprintf( __( '%1$sDocumentation%2$s. Our extensive documentation contains the API reference and some examples.', 'cart-rest-api-for-woocommerce' ), '<a href="' . esc_url( Help::build_shortlink( add_query_arg( $campaign_args, esc_url( COCART_DOCUMENTATION_URL ) ) ) ) . '" aria-label="' . esc_attr__( 'View API Reference', 'cart-rest-api-for-woocommerce' ) . '" title="' . esc_attr__( 'View API Reference', 'cart-rest-api-for-woocommerce' ) . '" target="_blank">', '</a>' ) . '</li>' .
					'<li>' . sprintf( __( '%1$sDiscussions%2$s. We have an active and friendly community on our %3$s server who may be able to help you figure out the how-tos of %4$s.', 'cart-rest-api-for-woocommerce' ), '<a href="' . esc_url( Help::build_shortlink( add_query_arg( $campaign_args, esc_url( COCART_STORE_URL . 'community/' ) ) ) ) . '" aria-label="' . esc_attr__( 'View API Reference', 'cart-rest-api-for-woocommerce' ) . '" title="' . esc_attr__( 'View API Reference', 'cart-rest-api-for-woocommerce' ) . '" target="_blank">', '</a>', 'Discord', 'CoCart' ) . '</li>' .
					'<li>' . sprintf( __( '%1$sTranslate%2$s. %3$s is in need of translations. Is the plugin not translated in your language or do you spot errors with the current translations? Helping out is easy! Head over to the project on WordPress.org and click %1$sTranslate %3$s%2$s.', 'cart-rest-api-for-woocommerce' ), '<a href="https://translate.wordpress.org/projects/wp-plugins/cart-rest-api-for-woocommerce" aria-label="' . esc_html__( 'Help translate', 'cart-rest-api-for-woocommerce' ) . '" title="' . esc_html__( 'Help translate', 'cart-rest-api-for-woocommerce' ) . ' "target="_blank">', '</a>', 'CoCart' ) . '</li>' .
					'<li>' . sprintf( __( '%1$sCommunity Forum%2$s. If you prefer, you can request for help on the WordPress Forum.', 'cart-rest-api-for-woocommerce' ), '<a href="' . esc_url( COCART_SUPPORT_URL ) . '" aria-label="' . esc_html__( 'Community Forum', 'cart-rest-api-for-woocommerce' ) . '" title="' . esc_html__( 'Community Forum', 'cart-rest-api-for-woocommerce' ) . '" target="_blank">', '</a>' ) . '</li>' .
					'<li>' . sprintf( __( '%2$sFound a bug?%4$s If you find a bug within %1$s core, please create a bug report on the %2$sGithub repository%4$s. Ensure you read the %3$scontribution guide%4$s prior to submitting your report. To help solve your issue as fast as possible, please be as descriptive as possible by filling in the template provided as requested.', 'cart-rest-api-for-woocommerce' ), 'CoCart', '<a href="https://github.com/co-cart/co-cart/issues" target="_blank">', '<a href="https://github.com/co-cart/co-cart/blob/trunk/.github/CONTRIBUTING.md" target="_blank">', '</a>' ) . '</li>' .
					'</ul>',
			) );
			...
			if ( strpos( trim( sanitize_key( wp_unslash( $_GET['page'] ) ) ), 'cocart-setup' ) !== 0 ) {
				$screen->add_help_tab( array(
					...
					'content' =>
						...
						'<p><a href="' . add_query_arg( array(
							'page' => 'cocart-setup',
						), admin_url( 'admin.php' ) ) . '" class="button button-primary" aria-label="' . sprintf( esc_attr__( 'View %s setup wizard', 'cart-rest-api-for-woocommerce' ), 'CoCart' ) . '">' . esc_html__( 'Setup Wizard', 'cart-rest-api-for-woocommerce' ) . '</a></p>',
				) );
			}
			...
			$screen->set_help_sidebar(
				...
				'<p><span class="dashicons dashicons-admin-home"></span> <a href="' . esc_url( Help::build_shortlink( add_query_arg( $campaign_args, esc_url( COCART_STORE_URL ) ) ) ) . '" target="_blank">' . esc_html__( 'Visit website', 'cart-rest-api-for-woocommerce' ) . '</a></p>' .
